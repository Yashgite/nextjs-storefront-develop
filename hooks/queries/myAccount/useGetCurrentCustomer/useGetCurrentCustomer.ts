...
//this sends graphql request at api/graphql
const loadUser = async () => {
  const response = await client.request({
    document: getCurrentUser, //graphql query
    variables: {},
  })
...
export const useGetCurrentCustomer = (isCSR?: boolean): UserResultType => {
  const { data, isLoading, isSuccess, isError, error } = useQuery({
    queryKey: loginKeys.user, // Cache key for React Query
    queryFn: loadUser, //function that makes API call
    enabled: !isCSR,
  })
